...
# /design-schema-aurora-mysql

Designs the Aurora MySQL target schema: resolved table/column types, DDL,
...
1. **Prepare input**
   ...
   ```bash
   uv run python scripts/run_schema_design.py --job-id {job_id} --db {database_name} --engine aurora_mysql --llm-mode external
   ```
...
3. **Trust the migration strategy (ADR-028 target rule)**
   ...
   - `carry_over` (source is MySQL): column types map 1:1; your value-add
...
4. **Design the schema**
   ...
   - Copy `primary_key`, `indexes`, and `foreign_keys` from the draft's
     matching table unchanged — the script already derives these from the
     source schema
...
5. **Write, validate, persist**
   Write to `.artifacts/{database_name}/{job_id}/llm_responses/schema_design_aurora_mysql.json`, then:
...
6. **Update state**
   Set `phase_status.schema_design_aurora_mysql` = "complete"
