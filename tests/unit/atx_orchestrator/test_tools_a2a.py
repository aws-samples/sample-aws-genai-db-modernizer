...
These test ``run_assessment_core_via_a2a`` without any network or SDK --
...
consolidated into one ``run_assessment_core_via_a2a`` that drives the
``db-modernization-assessment-core`` subagent. The subagent ticks its own
...
class TestRunDeterministicCoreViaA2AHappyPath:
    def test_returns_json_string_of_payload(self) -> None:
        ...
        with patch("src.atx_orchestrator.tools.invoke_and_wait", return_value=payload) as m:
            result_str = run_assessment_core_via_a2a(
                job_id="job-1",
    ...
    def test_invokes_correct_agent_name(self) -> None:
        """The tool must invoke the ``db-modernization-assessment-core`` agent."""
        with patch("src.atx_orchestrator.tools.invoke_and_wait", return_value={"ok": 1}) as m:
            run_assessment_core_via_a2a(job_id="job-1", database_name="discourse")
    ...
    def test_empty_input_key_defaults_correctly(self) -> None:
        with patch("src.atx_orchestrator.tools.invoke_and_wait", return_value={"ok": 1}) as m:
            run_assessment_core_via_a2a(job_id="job-1", database_name="db-1")
...
class TestRunDeterministicCoreViaA2AErrorPaths:
    @pytest.mark.parametrize(
        ...
        [
            (A2ATimeoutError, "A2A assessment-core failed"),
            (A2AFailedError, "A2A assessment-core failed"),
            (A2APayloadError, "A2A assessment-core failed"),
        ],
    )
    def test_a2a_errors_are_returned_as_json_error_dict(
        ...
        with patch("src.atx_orchestrator.tools.invoke_and_wait", side_effect=exc):
            result_str = run_assessment_core_via_a2a(job_id="job-1", database_name="db-1")
...
class TestToolsRegistered:
    def test_assessment_core_tool_in_pipeline_tools(self) -> None:
        ...
        assert "run_assessment_core_via_a2a" in tool_names
