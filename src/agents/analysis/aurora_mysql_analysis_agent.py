"""
Aurora MySQL Analysis Agent

Analyzes database workloads to identify queries that should stay on (or migrate to)
Aurora MySQL. Detects relational patterns where Aurora excels — complex joins,
aggregations, transactions, referential integrity, and MySQL-specific features
(JSON_EXTRACT, FULLTEXT MATCH AGAINST, ON DUPLICATE KEY, GROUP_CONCAT,
stored routines, multi-table UPDATE).
...
from src.tools.analysis.aurora_mysql_analysis_tools import (
    analyze_aurora_mysql_patterns,
    analyze_aurora_mysql_use_cases,
    build_aurora_mysql_decision_trace,
)


def analyze_for_aurora_mysql_deterministic(
    ...
    Returns:
        ...
        Third element is empty string — no Mermaid ER diagram for Aurora MySQL.
    """
    ...
    # === 1. Pattern detection ===
    workload_analysis = analyze_aurora_mysql_use_cases(collector_output)

    # === 2. Scoring + recommendations ===
    table_recommendations = analyze_aurora_mysql_patterns(collector_output, workload_analysis)
    ...
    # === 5. Decision trace ===
    decision_trace = build_aurora_mysql_decision_trace(
...
def prepare_aurora_mysql_llm_input(
...
def apply_aurora_mysql_llm_output(
    deterministic_result: AnalysisOutputContract,
    llm_output,
...
def analyze_for_aurora_mysql(
    analysis_input: AnalysisInput,
    llm_mode: str = "none",
) -> tuple[AnalysisOutputContract, dict, str]:
    """Main entry point for Aurora MySQL analysis agent.
    ...
    """
    if llm_mode == "none":
        return analyze_for_aurora_mysql_deterministic(analysis_input)

    if llm_mode == "external":
        contract, trace, mermaid = analyze_for_aurora_mysql_deterministic(analysis_input)
