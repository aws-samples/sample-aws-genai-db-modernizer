"""Aurora PostgreSQL schema design agent — uses SchemaDesignRunner for PE review loop.
...
# ---------------------------------------------------------------------------
# Module-level path variables (set by run_aurora_postgresql_schema_agent, read by tools)
# ---------------------------------------------------------------------------
...
DEFAULT_SKILL_PATH = str(
    Path(__file__).resolve().parent.parent.parent / "skills" / "aurora_postgresql-data-modeling.md"
)
DEFAULT_PE_SKILL_PATH = str(
    Path(__file__).resolve().parent.parent.parent / "skills" / "aurora_postgresql-pe-review.md"
)


# ---------------------------------------------------------------------------
# Aurora PostgreSQL-specific PE Review models
# ---------------------------------------------------------------------------
...
class PEReviewResult(BaseModel):
    """Aurora PostgreSQL PE review result — compatible with SchemaDesignRunner."""
...
def load_agent_input() -> dict:
    ...
    Reads from paths set by run_aurora_postgresql_schema_agent (preferred) or
    env vars (fallback).
    """
    ...
    if not collector_path or not analysis_path:
        raise ValueError(
            "Collector/analysis paths must be set via run_aurora_postgresql_schema_agent() "
...
def _invoke_pe_reviewer(
    model: BedrockModel,
    design_output: AuroraPostgresqlModelOutputContract,
    agent_input_summary: dict,
    pe_skill_path: str | None = None,
) -> PEReviewResult:
    """Invoke the PE reviewer agent on a design output."""
    print("[schema-design/aurora_postgresql][pe-review] Invoking PE reviewer...")
    ...
    prompt = (
        "Review the following Aurora PostgreSQL schema design.\n\n"
    ...
    if isinstance(output, PEReviewResult):
        print(
            f"[schema-design/aurora_postgresql][pe-review] Verdict: {output.verdict.value} | "
...
def run_aurora_postgresql_schema_agent(
    skill_path: str | None = None,
    pe_skill_path: str | None = None,
    collector_path: str | None = None,
    analysis_path: str | None = None,
    revision_context_path: str | None = None,
) -> tuple[AuroraPostgresqlModelOutputContract, dict]:
    """Run the Aurora PostgreSQL schema design agent with PE review loop.
    ...
    print("[schema-design/aurora_postgresql] Loading agent input...")
    ...
    print(f"[schema-design/aurora_postgresql] Compacted input: {len(input_json):,} chars")

    designer = Agent(
        model=model,
        system_prompt=system_prompt,
        tools=[],
        structured_output_model=AuroraPostgresqlModelOutputContract,
        callback_handler=None,
    )

    designer_prompt = (
        "Here is the projected input plus the deterministic draft for your "
        "Aurora PostgreSQL schema design:\n\n"
        f"```json\n{input_json}\n```\n\n"
    ...
    runner = SchemaDesignRunner(
        target_type="aurora_postgresql",
        output_model=AuroraPostgresqlModelOutputContract,
        model=model,
