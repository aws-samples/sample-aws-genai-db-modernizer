"""Aurora PostgreSQL schema-design output contract.
...
class TargetColumn(BaseModel):
    ...
    aurora_type: str = Field(..., description="Resolved Aurora PostgreSQL type, e.g. VARCHAR(255)")
...
class TargetTable(BaseModel):
    table_name: str = Field(..., description="Target Aurora PostgreSQL table name")
...
class AuroraPostgresqlModelOutputContract(BaseModel):
    """Output contract for the Aurora PostgreSQL schema-design agent."""
    ...
    target_engine: str = Field(default="aurora_postgresql", description="Target engine identifier")
    ...
    table_definitions: list[TargetTable] = Field(
        ..., min_length=1, description="Aurora PostgreSQL table designs"
    )
